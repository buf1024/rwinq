...
class ChipDistribution():
    ...
    def calcu(self, dateT, highT, lowT, avgT, volT, TurnoverRateT, minD=0.01, flag=1, AC=1):
        if flag == 1:
            self.calcuSin(dateT, highT, lowT, avgT, volT,
                          TurnoverRateT, A=AC, minD=minD)
        elif flag == 2:
            self.calcuJUN(dateT, highT, lowT, volT,
                          TurnoverRateT, A=AC, minD=minD)
    ...
    def calcuChip(self, flag=1, AC=1):  # flag 使用哪个计算方式,    AC 衰减系数
        ...
        for i in range(len(date)):
            ...
            # print(date[i])
            dateT = date[i]
            self.calcu(dateT, highT, lowT, avgT, volT, TurnoverRateT /
                       100, flag=flag, AC=AC)  # 东方财富的小数位要注意，兄弟萌。我不除100懵逼了
...
if __name__ == "__main__":
    ...
    async def test():
        a = ChipDistribution()
        data = await fetch.fetch_stock_bar(code='sz301421')
        data = data['bars']
        data['avg'] = data['amount']/data['volume']/100.0
