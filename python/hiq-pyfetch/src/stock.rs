...
pub(crate) struct StockFetch {
    fetch: Arc<dyn hiq_fetch::StockFetch + Send + 'static>,
}
...
impl StockFetch {
    ...
    fn new() -> Self {
        StockFetch {
            fetch: Arc::new(hiq_fetch::stock_fetch()),
        }
    }
    /// 股票大盘指数（列举几个而已）
...
pub(crate) struct BlockStockFetch {
    fetch: Box<dyn hiq_fetch::StockFetch + Send + 'static>,
}
...
impl BlockStockFetch {
    ...
    fn new() -> Self {
        BlockStockFetch {
            fetch: Box::new(hiq_fetch::stock_fetch()),
        }
    }
    /// 股票大盘指数（列举几个而已）
