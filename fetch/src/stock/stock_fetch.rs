...
fn get_cell(cell: &DataType) -> String {
    match cell {
        DataType::Int(cont) => {
            format!("{:0>6}", cont)
        }
...
/// 股票大盘指数（列举几个而已）
pub async fn fetch_index_info() -> Result<Vec<StockInfo>> {
    let data = vec![
        ("sh000001", "上证综指"),
        ("sz399001", "深证成指"),
        ("sz399006", "创业板指"),
...
pub async fn fetch_index_bar(
    ...
    fetch_stock_bar(code, name, freq, start, end, skip_rt).await
}

async fn fetch_stock_info_sh(margin_codes: &HashSet<String>) -> Result<Vec<StockInfo>> {
...
async fn fetch_stock_info_sz(margin_codes: &HashSet<String>) -> Result<Vec<StockInfo>> {
    let mut data = Vec::new();
    ...
    let resp = async_client().get(req_url).send().await?.bytes().await?;
    ...
    if let Some(Ok(range)) = workbook.worksheet_range("A股列表") {
        let tmp_vec: Vec<_> = range
            ...
            .map(|row| {
                ...
                let code = to_std_code(MarketType::Stock, &get_cell(&row[4]));
                let listing_date =
                    NaiveDate::parse_from_str(&get_cell(&row[6]), "%Y-%m-%d").unwrap();
                let listing_date =
                    NaiveDateTime::new(listing_date, NaiveTime::from_hms_opt(0, 0, 0).unwrap());
                StockInfo {
                    code: code.clone(),
                    name: get_cell(&row[5]),
                    block: get_cell(&row[0]),
                    is_margin: margin_codes.contains(&code),
                    listing_date,
                }
            })
...
async fn fetch_stock_info_bj(margin_codes: &HashSet<String>) -> Result<Vec<StockInfo>> {
    let mut data = Vec::new();
...
/// 获取股票基本信息
pub async fn fetch_stock_info(market: Option<Market>) -> Result<Vec<StockInfo>> {
    let margin_codes = fetch_stock_is_margin().await?;

    let data = if let Some(m) = market {
        let mut data = Vec::new();
        match m {
            Market::SZ => data.extend(fetch_stock_info_sz(&margin_codes).await?.into_iter()),
            Market::SH => data.extend(fetch_stock_info_sh(&margin_codes).await?.into_iter()),
            Market::BJ => data.extend(fetch_stock_info_bj(&margin_codes).await?.into_iter()),
        }
        data
    } else {
        let mut data = Vec::new();
        data.extend(fetch_stock_info_sh(&margin_codes).await?.into_iter());
        data.extend(fetch_stock_info_sz(&margin_codes).await?.into_iter());
        data.extend(fetch_stock_info_bj(&margin_codes).await?.into_iter());
        data
    };
    Ok(data)
}
/// 获取融资融券股票代码
pub async fn fetch_stock_is_margin() -> Result<HashSet<String>> {
    let mut page = 1;
...
/// 股票最新指标
pub async fn fetch_stock_index(date: Option<NaiveDate>) -> Result<HashMap<String, StockIndex>> {
...
/// 股票行业
pub async fn fetch_stock_industry() -> Result<Vec<StockIndustry>> {
    let req_url = format!(
...
/// 股票概念
pub async fn fetch_stock_concept() -> Result<Vec<StockConcept>> {
    let req_url = format!(
        "http://push2.eastmoney.com/api/qt/clist/get?\
        pn=1&pz=2000&po=1&np=1&ut=bd1d9ddb04089700cf9c27f6f7426281&fltt=2&invt=2&fid=f3&\
...
/// 股票业绩报表
pub async fn fetch_stock_yjbb(year: u16, season: u16) -> Result<Vec<StockYJBB>> {
    ...
    loop {
        ...
        let tmp_vec: Vec<_> = result
            ...
            .map(|item| {
                let season_date =
                    NaiveDateTime::parse_from_str(item.season_date, "%Y-%m-%d %H:%M:%S").unwrap();
                StockYJBB {
                    year,
                    season,
                    season_date,
...
/// 股票排名
pub async fn fetch_stock_hot_rank(code: &str) -> Result<StockHotRank> {
...
/// 全量股票行情，注意调用频率
pub async fn fetch_stock_rt_quot(codes: Option<Vec<String>>) -> Result<Vec<StockRtQuot>> {
    let req_url = format!(
...
/// 千股千评
pub async fn fetch_stock_comment(codes: Option<Vec<String>>) -> Result<Vec<StockComment>> {
    let mut page = 1;
    let page_size: i32 = 500;
    let mut data = Vec::new();
    let mut total_page = 0;
...
// async fetch_attention_rank_data(code: String) ->
/// 千股千评历史
pub async fn fetch_stock_comment_his(code: String) -> Result<Vec<StockComment>> {
    // 机构参与度 / 市场成本
...
mod tests {
    ...
    fn test_fetch_stock_info() {
        tokio::runtime::Builder::new_multi_thread()
            ...
            .unwrap()
            .block_on(async {
                let data = fetch_stock_info(None).await;
                assert!(data.is_ok());
    ...
    fn test_fetch_stock_is_margin() {
        tokio::runtime::Builder::new_multi_thread()
            ...
            .unwrap()
            .block_on(async {
                let data = fetch_stock_is_margin().await;
                assert!(data.is_ok());
    ...
    fn test_fetch_stock_bar() {
        tokio::runtime::Builder::new_multi_thread()
            ...
            .block_on(async {
                ...
                let data = fetch_stock_bar("sz000001", None, None, Some(start), None, false).await;
    ...
    fn test_fetch_stock_index() {
        tokio::runtime::Builder::new_multi_thread()
            ...
            .unwrap()
            .block_on(async {
                let data = fetch_stock_index(None).await;
                assert!(data.is_ok());
    ...
    fn test_fetch_stock_industry() {
        tokio::runtime::Builder::new_multi_thread()
            ...
            .unwrap()
            .block_on(async {
                let data = fetch_stock_industry().await;
                assert!(data.is_ok());
    ...
    fn test_fetch_stock_industry_detail() {
        tokio::runtime::Builder::new_multi_thread()
            ...
            .unwrap()
            .block_on(async {
                let data = fetch_stock_industry_detail(None, None).await;
                assert!(data.is_ok());
    ...
    fn test_fetch_stock_industry_daily() {
        tokio::runtime::Builder::new_multi_thread()
            ...
            .unwrap()
            .block_on(async {
                let data = fetch_stock_industry_daily("BK1044", None, None, None, true).await;
                assert!(data.is_ok());
                let data = data.unwrap();
                let data = data.bars;
                assert!(data.is_some());
    ...
    fn test_fetch_stock_concept() {
        tokio::runtime::Builder::new_multi_thread()
            ...
            .unwrap()
            .block_on(async {
                let data = fetch_stock_concept().await;
                assert!(data.is_ok());
    ...
    fn test_fetch_stock_concept_detail() {
        tokio::runtime::Builder::new_multi_thread()
            ...
            .unwrap()
            .block_on(async {
                let data = fetch_stock_concept_detail(Some("BK1109"), None).await;
                assert!(data.is_ok());
    ...
    fn test_fetch_stock_concept_daily() {
        tokio::runtime::Builder::new_multi_thread()
            ...
            .unwrap()
            .block_on(async {
                let data = fetch_stock_concept_daily("BK0969", None, None, None, true).await;
                assert!(data.is_ok());
                let data = data.unwrap();
                let data = data.bars;
                assert!(data.is_some());
    ...
    fn test_fetch_stock_yjbb() {
        tokio::runtime::Builder::new_multi_thread()
            ...
            .unwrap()
            .block_on(async {
                let data = fetch_stock_yjbb(2022, 3).await;
                assert!(data.is_ok());
                let data = data.unwrap();
                assert!(data.len() > 0);
    ...
    fn test_fetch_stock_margin() {
        tokio::runtime::Builder::new_multi_thread()
            ...
            .block_on(async {
                ...
                let data = fetch_stock_margin("sh601928", Some(start), Some(end)).await;
                assert!(data.is_ok());
                let data = data.unwrap();
                assert!(data.len() > 0);
    ...
    fn test_fetch_stock_hot_rank() {
        tokio::runtime::Builder::new_multi_thread()
            ...
            .unwrap()
            .block_on(async {
                let data = fetch_stock_hot_rank("sz300468").await;
                assert!(data.is_ok());
                let data = data.unwrap();
    ...
    fn test_fetch_stock_rt_quot() {
        tokio::runtime::Builder::new_multi_thread()
            ...
            .block_on(async {
                let data =
                    fetch_stock_rt_quot(Some(vec!["sz000001".into(), "sh603270".into()])).await;
                if data.is_err() {
    ...
    fn test_fetch_stock_comment() {
        tokio::runtime::Builder::new_multi_thread()
            ...
            .unwrap()
            .block_on(async {
                let data = fetch_stock_comment(None).await;
                if data.is_err() {
                    println!("{:?}", data.as_ref().err().unwrap());
                }
                assert!(data.is_ok());
    ...
    fn test_fetch_stock_comment_his() {
        tokio::runtime::Builder::new_multi_thread()
            ...
            .unwrap()
            .block_on(async {
                let data = fetch_stock_comment_his("sh600000".into()).await;
                if data.is_err() {
                    println!("{:?}", data.as_ref().err().unwrap());
                }
                assert!(data.is_ok());
